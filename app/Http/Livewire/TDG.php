...
class TDG extends Component
{
    ...
    public $soPhieu, $soPhieuXacNhanDonHang, $uncheckData, $soThapPhan, $danhSachMailSale, $danhSachMailSaleAll , $fileExcel, $dataFileExcel, $dataFileExcelUnique, $note;
    ...
    public function mount(){
        ...
        if(Auth::user()->hasAnyPermission(['approve_1_tdg', 'approve_2_tdg', 'approve_3_tdg', 'approve_4_tdg', 'create_tdg']) || Auth::user()->hasRole('sale')){
    ...
    public function createTDGNewVersionModal(){
    ...
    public function createTDGNewVersion(){
        ...
        $this->soPhieu = IdGenerator::generate(['table' => 'phieu_tdg', 'field' => 'so_phieu', 'length' => '13', 'prefix' => 'TDG-' . Carbon::now()->isoFormat('DDMMYY') . '-','reset_on_prefix_change' => true]);

        DB::transaction(function() use($contentMailFinish) {

            foreach ($this->spec as $key=>$value) {
            
                DB::table('phieu_tdg')->insert([

                    'so_phieu' => $this->soPhieu,
                    'so_phieu_xac_nhan_don_hang' => $this->soPhieuXacNhanDonHang,
                    'customer' => $this->buyer,
            ...
            DB::table('phieu_tdg_content')->insert([
        ...
        $activeWorksheet->setCellValue('G1', 'TDG Number'); $activeWorksheet->setCellValue('H1', $this->soPhieu); $activeWorksheet->setCellValue('I1', Carbon::now()->isoFormat('DD.MM.YYYY'));
        ...
        $activeWorksheet->setCellValue('G' . $currentRow + 19, 'APPROVAL LEVEL - 04');
        ...
        $activeWorksheet->setCellValue('G' . $currentRow + 20, 'Time 4');
        ...
        $activeWorksheet->setCellValue('G' . $currentRow + 21, 'User 4');
        ...
        $activeWorksheet->setCellValue('G' . $currentRow + 22, 'User 4');
        ...
        $activeWorksheet->mergeCells('G' . $currentRow + 19 . ':I' . $currentRow + 19);
        ...
        $activeWorksheet->mergeCells('G' . $currentRow + 20 . ':I' . $currentRow + 20);
        ...
        $activeWorksheet->getStyle('A' . $currentRow + 19 . ':I' . $currentRow + 22)->applyFromArray($styleArray);
        ...
        $user = User::permission('approve_1_tdg')->first();

        Mail::to($user->email)
        ->cc($ccMail)
        ->send(new MailTDG($this->soPhieu, 'Approve 1', $this->buyer, $contentMailFinish, Carbon::now()));

        flash()->addSuccess('Tạo TDG thành công.');

        $this->resetInputField();

        $this->emit('createTDGNewVersionModal');
    ...
    public function createTDG(){
        ...
        if($dataFileExcel != null){
            ...
            . $PalletTemp . $CommissionTemp . $OtherTemp . $StockInYearTemp . $this->note
            ...
            //     ->where('so_phieu', $this->soPhieuXacNhanDonHang)
            //     ->first();

            // if($XNDH == null){

            //     sweetalert()->addError('Số phiếu Xác nhận đơn hàng không tồn tại.');
            //     return;

            // }else{

            //     $TDG = DB::table('phieu_tdg')
            //         ->where('so_phieu_xac_nhan_don_hang', $this->soPhieuXacNhanDonHang)
            //         ->first();

            //     if($TDG != null){

            //         sweetalert()->addError('Số phiếu Xác nhận đơn hàng đã được sử dụng.');
            //         return;
            ...
            $activeWorksheet->setCellValue('G10', 'Zpr0'); $activeWorksheet->setCellValue('H10', 'Zpr0');
            ...
            $activeWorksheet->setCellValue('A' . $row + 3,'Total Expense'); $activeWorksheet->setCellValue('B' . $row + 3, '=SUM(B' . $row + 4 . ':B' . $row + 10 . ')'); $activeWorksheet->setCellValue('C' . $row + 3,'=SUM(C' . $row + 4 . ':C' . $row + 10 . ')');
            ...
            $activeWorksheet->setCellValue('A' . $row + 10,'Other fee'); $activeWorksheet->setCellValue('B' . $row + 10,$otherFeeUSDEXCEL); $activeWorksheet->setCellValue('C' . $row + 10,$otherFeeVNDEXCEL);

            $activeWorksheet->setCellValue('A' . $row + 12, 'Remark');
            $activeWorksheet->setCellValue('A' . $row + 13, $addressEXCEL);

            $activeWorksheet->setCellValue('A' . $row + 15, 'Note');
            $activeWorksheet->setCellValue('A' . $row + 16, $this->note);

            $activeWorksheet->setCellValue('A' . $row + 18, 'Buyer name:'); $activeWorksheet->setCellValue('B' . $row + 18, $this->buyerName);
            $activeWorksheet->setCellValue('A' . $row + 19, 'Consignee: '); $activeWorksheet->setCellValue('B' . $row + 19, $this->consignee);
            $activeWorksheet->setCellValue('A' . $row + 20, 'Commission and agency name:'); $activeWorksheet->setCellValue('B' . $row + 20, $this->commission);
            $activeWorksheet->setCellValue('A' . $row + 21, 'End use:'); $activeWorksheet->setCellValue('B' . $row + 21, $this->endUse);
            $activeWorksheet->setCellValue('A' . $row + 22, 'Fabric construction:'); $activeWorksheet->setCellValue('B' . $row + 22, $this->fabricConstruction);
            $activeWorksheet->setCellValue('A' . $row + 23, '1 set of yarn:'); $activeWorksheet->setCellValue('B' . $row + 23, $this->_1setOfYarn);
            $activeWorksheet->setCellValue('A' . $row + 24, 'Incoterms:'); $activeWorksheet->setCellValue('B' . $row + 24, $this->incoterms);
            $activeWorksheet->setCellValue('A' . $row + 25, 'Payment terms:'); $activeWorksheet->setCellValue('B' . $row + 25, $this->paymentTerms);
            $activeWorksheet->setCellValue('A' . $row + 26, 'Payment Date:'); $activeWorksheet->setCellValue('B' . $row + 26, $this->paymentDate);
            $activeWorksheet->setCellValue('A' . $row + 27, 'Delivery Date:'); $activeWorksheet->setCellValue('B' . $row + 27, $this->deliveryDate);
            $activeWorksheet->setCellValue('A' . $row + 28, 'Brand name:'); $activeWorksheet->setCellValue('B' . $row + 28, $this->brandName);
            $activeWorksheet->setCellValue('A' . $row + 29, 'Packing:'); $activeWorksheet->setCellValue('B' . $row + 29, $this->packing);
            $activeWorksheet->setCellValue('A' . $row + 30, 'Truck fee:'); $activeWorksheet->setCellValue('B' . $row + 30, $this->truckFee);
            $activeWorksheet->setCellValue('A' . $row + 31, 'T/C:'); $activeWorksheet->setCellValue('B' . $row + 31, $this->tc);
            $activeWorksheet->setCellValue('A' . $row + 32, 'C/O:'); $activeWorksheet->setCellValue('B' . $row + 32, $this->co);
            ...
            Storage::disk('ftp')->put('TDG/' . $this->soPhieu . ".xlsx", $content); 
            ...
            Mail::to($user->email)
            ->cc($ccMail)
            ->send(new MailTDG($this->soPhieu, 'Approve 1', $customer, $contentMailFinish, Carbon::now()));

            flash()->addSuccess('Tạo TDG thành công.');

            $this->resetInputField();

            $this->emit('createTDGModal');
    ...
    public function duyetWebModal($soPhieu){
        ...
        $this->khachHang = $phieuTDG->customer;
    ...
    public function duyetWeb(){
        ...
        if($phieuTDG->status == 'New'){

            if(Auth::user()->hasPermissionTo('approve_1_tdg')){
                ...
                $user = User::permission('approve_2_tdg')->first();
    
                Mail::to($user->email)
                ->cc($ccMail)
                ->send(new MailTDG($this->soPhieu, 'Approve 2', $phieuTDG->customer, $content->content, Carbon::now()));
        ...
        }elseif($phieuTDG->status == 'Approve 1'){

            if(Auth::user()->hasPermissionTo('approve_2_tdg')){
                ...
                $user = User::permission('approve_3_tdg')->first();
    
                Mail::to($user->email)
                ->cc($ccMail)
                ->send(new MailTDG($this->soPhieu, 'Approve 3', $phieuTDG->customer, $content->content, Carbon::now()));
                flash()->addSuccess('Thành công.');
                $this->emit('duyetWebModal');
        ...
        }elseif($phieuTDG->status == 'Approve 2'){

            if(Auth::user()->hasPermissionTo('approve_3_tdg')){
                ...
                Mail::to('md@century.vn')->send(new MailTDG($this->soPhieu, 'Approve 4', $phieuTDG->customer, $content->content, Carbon::now()));
                flash()->addSuccess('Thành công.');
                $this->emit('duyetWebModal');
        ...
        }elseif($phieuTDG->status == 'Approve 3'){

            if(Auth::user()->hasPermissionTo('approve_4_tdg')){

                DB::transaction(function() use ($phieuTDG){

                    DB::table('phieu_tdg')
                    ->where('so_phieu', $this->soPhieu)
                    ->update([
        
                        'approved_4' => Auth::user()->username,
                ...
                ->cc($ccMail)
                ->send(new MailTDG($this->soPhieu, 'Finish', $phieuTDG->customer, $content->content, Carbon::now()));
    ...
    public function reject(){
        ...
        if($phieuTDG->status == 'New'){
            ...
            if(Auth::user()->hasPermissionTo('approve_1_tdg')){
                ...
                Mail::to($content->mail_to)
                ->cc($ccMail)
                ->send(new MailTDG($this->soPhieu, 'Reject', $phieuTDG->customer, $content->content, Carbon::now()));
                flash()->addSuccess('Thành công.');
    ...
    public function detailModal($soPhieu){
        ...
        $this->log = DB::table('phieu_tdg_log')
        ->where('so_phieu', $soPhieu)
        ->get();
    ...
    public function downloadFile($soPhieu){
        ...
        return Storage::disk('ftp')->download('TDG/' . $soPhieu . '/' . $soPhieu. '.xlsx');
    }
    ...
    public function render()
    {
        if($this->state == 'create'){
            ...
            if($this->danhSachMailSale == null)
                $this->danhSachMailSale =  User::role(['sale_admin'])->get();
        ...
        // ->select('so_phieu', 'customer', 'valid_from', 'valid_to', 'created_user', 'status')->distinct('so_phieu')->paginate($this->paginate);

        $search_fields = [
            'phieu_tdg.so_phieu',
            'phieu_tdg.customer',
            'phieu_tdg.created_user',
            'phieu_tdg.status',
        ];
        ...
        ->where('phieu_tdg.is_delete', null)

        ->where( function ($query){

            if($this->tuNgay == null && $this->denNgay == null){

                $query->whereBetween('phieu_tdg.created_at', ['2023-05-01 00:00:00', '2099-01-01 00:00:00']);
    
            }elseif($this->tuNgay != null && $this->denNgay != null){
    
                $query->whereBetween('phieu_tdg.created_at', [$this->tuNgay . ' 00:00:00', $this->denNgay . ' 23:59:59']);
    
            }elseif($this->tuNgay == null && $this->denNgay != null){
    
                $query->whereBetween('phieu_tdg.created_at', ['2023-05-01 00:00:00', $this->denNgay . ' 23:59:59']);
    
            }
            elseif($this->tuNgay != null && $this->denNgay == null){
    
                $query->whereBetween('phieu_tdg.created_at', [$this->tuNgay . ' 00:00:00', '2099-01-01 00:00:00']);
            ...
            if($this->canhan_tatca == 'phieuDoiDuyet'){
                ...
                if(Auth::user()->hasPermissionTo('approve_1_tdg')){
                    ...
                    $query->orWhere(function($query){
                        ...
                        $query->where('phieu_tdg.status', 'New');
                ...
                if(Auth::user()->hasPermissionTo('approve_2_tdg')){
                    ...
                    $query->orWhere(function($query){
                        ...
                        $query->where('phieu_tdg.status', 'Approve 1');
                ...
                if(Auth::user()->hasPermissionTo('approve_3_tdg')){
                    ...
                    $query->orWhere(function($query){
                        ...
                        $query->where('phieu_tdg.status', 'Approve 2');
                ...
                if(Auth::user()->hasPermissionTo('approve_4_tdg')){
                    ...
                    $query->orWhere(function($query){
                        ...
                        $query->where('phieu_tdg.status', 'Approve 3');

                    });

                }

                if(Auth::user()->hasPermissionTo('create_tdg')){

                    $query->orWhere(function($query){

                        $query->whereNotIn('phieu_tdg.status', ['Finish', 'Reject 1', 'Reject 2', 'Reject 3', 'Reject 4']);

                        $query->where('phieu_tdg.created_user', Auth::user()->username);
                ...
                if(Auth::user()->hasRole('sale')){
                    ...
                    $query->orWhere(function($query){
                        ...
                        $query->where('phieu_tdg.status', '<>', 'Finish');

                        $query->where('phieu_tdg_content.mail_to', Auth::user()->email);
                ...
                $query->where(function($query){
                    ...
                    $query->orWhere(function($query){
                        ...
                        $query->where('phieu_tdg.new', Auth::user()->username);
                        $query->where('phieu_tdg.status', 'Finish');

                    });
                    $query->orWhere('phieu_tdg.approved_1', Auth::user()->username);
                    $query->orWhere('phieu_tdg.approved_2', Auth::user()->username);
                    $query->orWhere('phieu_tdg.approved_3', Auth::user()->username);
                    $query->orWhere('phieu_tdg.approved_4', Auth::user()->username);
        ...
        $danhSachTDG->select('phieu_tdg.so_phieu', 'phieu_tdg.customer', 'phieu_tdg_content.mail_to', 'phieu_tdg.valid_from', 'phieu_tdg.valid_to', 'phieu_tdg.created_user', 'phieu_tdg.status')->distinct('phieu_tdg.so_phieu');
                
        $result = $danhSachTDG->paginate($this->paginate);

        return view('livewire.t-d-g', compact('result'));
    }
}
