...
class TDG extends Mailable
{
    ...
    public function envelope(): Envelope
    {
        if($this->status == 'Approve 1'){
            ...
            return new Envelope(
                subject: 'TDG ' . $this->customer . ' duyệt cấp 1',
            );

        }elseif($this->status == 'Approve 2'){

            return new Envelope(
                subject: 'TDG ' . $this->customer . ' duyệt cấp 2',
            );

        }elseif($this->status == 'Approve 3'){

            return new Envelope(
                subject: 'TDG ' . $this->customer . ' duyệt cấp 3',
            );

        }elseif($this->status == 'Approve 4'){

            return new Envelope(
                subject: 'TDG ' . $this->customer . ' duyệt cấp 4',
            );

        }elseif($this->status == 'Finish'){

            return new Envelope(
                subject: 'TDG ' . $this->customer . ' đã duyệt thành công.',
            );

        }elseif($this->status == 'Reject'){

            return new Envelope(
                subject: 'TDG ' . $this->customer . ' đã bị từ chối.',
            );
